...
export function get(
  ...
  try {
    ...
    const complexType = getComplexType(
      ast,
      propertyAst,
      importedPropTypes,
      options
...
function getSimpleType(
  ...
  const [required, simpleTypeName] = getSimpleTypeName(
    ast,
    propertyAst,
    importedPropTypes
  );
...
function getComplexType(
  ...
  const [required, complexTypeName, typeAst] = getComplexTypeName(
    ast,
    propertyAst,
    importedPropTypes
  );
  switch (complexTypeName) {
    case 'instanceOf':
      return getTypeDeclaration(
        dom.create.namedTypeReference(typeAst.arguments[0].name),
        !required
      );
    case 'oneOfType':
      const typeDecls = typeAst.arguments[0].elements.map((subtree: any) =>
    ...
    case 'oneOf':
      ...
      return getTypeDeclaration(
        dom.create.union(enumEntries as dom.Type[]),
        !required
      );
    case 'shape':
      const entries = getShapeProperties(ast, typeAst.arguments[0]).map(
        (entry: any) => {
...
function isRequired(ast: AstQuery, propertyAst: any): [boolean, any] {
  const required = ast.querySubtree(
    propertyAst,
    `
...
function getSimpleTypeName(
  ...
  if (!propTypesName && typeAst.type === 'Identifier') {
    const propType = propTypes.find(
      ({ localName }) => localName === typeAst.name
    );
...
function getComplexTypeName(
  ...
  if (typeAst.type === 'CallExpression') {
    const [, simpleTypeName] = getSimpleTypeName(
      ast,
      typeAst.callee,
      importedPropTypes
    );
